...
อ้างอิง: spec.md SPEC-BKG-001 Draft v2 | Updated: 2569-09-16 | สร้างด้วย /plan แล้วทีมตรวจแล้ว (plan v1)

## 1. สรุปแนวทาง
- ผู้รับบริการที่ยืนยันตัวตนแล้ว ค้นช่วงเวลาว่าง เลือก แล้วยืนยันการจอง ได้หมายเลขคิวกลับทันที
- หลังบ้านเป็น API (Python FastAPI) เก็บข้อมูลใน MySQL ตาม CON-TECH-01 ผ่าน SQLAlchemy
...
|---|---|---|
| MySQL 8 | CON-TECH-01 | บังคับโดยฝ่าย IT โรงพยาบาล ใช้ในระบบจริง |
...
| pytest | ทีมเลือกเอง ไม่ได้มาจาก spec | ตอน test ใช้ SQLite ในหน่วยความจำ (`sqlite:///:memory:`) แทน MySQL เพราะใน Codespace ไม่มี MySQL |
...
- รายการค้างส่งข้อความ (ASM-03) เก็บในคิว Redis ไม่ใช่ตารางใน MySQL
...
|---|---|---|
| CON-TECH-01 | ข้อ 2 และข้อ 3 (ตารางทั้งหมดอยู่ใน MySQL ในระบบจริง) | ใช้แล้ว |
